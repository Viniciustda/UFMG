...
class Parser:
    ...
    def parse(self):
        ...
        """
        """
        Método principal do parser que redireciona para a análise apropriada.
        """
        if self.current_token() and self.current_token().kind == TokenType.FUN:
            return self.parse_fun()  # Funções recursivas
        return self.parse_fn()  # Outras expressões
    
    def parse_fun(self):
        """
        Analisa a definição de funções recursivas nomeadas.
        Exemplo
        """
        if self.current_token() and self.current_token().kind == TokenType.FUN:
            self.consume_token()  # Consome 'fun'

            # Nome da função
            func_name = self.consume_token()
            if func_name.kind != TokenType.VAR:
                raise ValueError("Expected function name after 'fun'")

            # Parâmetro formal
            param_name = self.consume_token()
            if param_name.kind != TokenType.VAR:
                raise ValueError("Expected parameter name after function name")

            # Consome o operador '='
            assign_op = self.consume_token()
            if assign_op.kind != TokenType.EQL:
                raise ValueError("Expected '=' after function parameter")

            # Corpo da função: pode conter expressões complexas
            body = self.parse_fn()

            # Retorna a função recursiva como um objeto Fun
            # 'Fun' encapsula o nome da função, o parâmetro e o corpo
            return Fun(func_name.text, param_name.text, body)

        raise ValueError("Invalid 'fun' declaration")
    ...
    def parse_factor(self):
        ...
        while self.current_token() and self.current_token().kind in (TokenType.MUL, TokenType.DIV, TokenType.MOD):
            operator = self.consume_token()
            ...
            if operator.kind == TokenType.MUL:
                ...
                left = Div(left, right)
            elif operator.kind == TokenType.MOD:
                left = Mod(left, right)
    ...
    def parse_let_expression(self):
        """
        Analisa a expressão 'let-in-end', incluindo declarações 'val' e 'fun'.
        """
        if self.current_token() and self.current_token().kind == TokenType.LET:
            self.consume_token()  # Consome 'let'

            # Processa declarações
            decl_type = self.consume_token()
            if decl_type.kind not in [TokenType.VAL, TokenType.FUN]:
                raise ValueError("Expected 'val' or 'fun' after 'let'")

            if decl_type.kind == TokenType.VAL:
                # Processa declaração 'val'
                var_name = self.consume_token()
                if var_name.kind != TokenType.VAR:
                    raise ValueError("Expected variable name after 'val'")
                ...
                # Processa 'in' e 'end'
                if not self.current_token() or self.current_token().kind != TokenType.INX:
                    raise ValueError("Expected 'in' after declaration")
                self.consume_token()

                body = self.parse_fn()

                if not self.current_token() or self.current_token().kind != TokenType.END:
                    raise ValueError("Expected 'end' at the end of 'let' expression")
                self.consume_token()

                return Let(var_name.text, expr_value, body)

            elif decl_type.kind == TokenType.FUN:
                # Processa declaração 'fun'
                self.cur_token_idx -= 1  # Volta para processar com parse_fun
                func_decl = self.parse_fun()

                # 'in'
                if not self.current_token() or self.current_token().kind != TokenType.INX:
                    raise ValueError("Expected 'in' after 'fun' declaration")
                self.consume_token()
                ...
                # 'end'
                if not self.current_token() or self.current_token().kind != TokenType.END:
                    raise ValueError("Expected 'end' after 'let' expression")
                self.consume_token()

                # Gera um 'Let' onde o nome da função está no ambiente
                return Let(func_decl.name, func_decl, body)
        else:
