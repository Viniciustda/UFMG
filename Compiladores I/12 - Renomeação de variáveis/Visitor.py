...
class RenameVisitor(ABC):
    ...
    def visit_let(self, exp, arg):
        ...
        previous_mapping = arg.get(original_name)

        exp.exp_def.accept(self, arg)
        
        arg[original_name] = new_name
        exp.identifier = new_name
        
        exp.exp_body.accept(self, arg)
        
        if previous_mapping is None:
            del arg[original_name]  
        else:
            arg[original_name] = previous_mapping
...
class GenVisitor(Visitor):
    ...
    def visit_not(self, exp, prog):
        ...
        v_name = self.next_var_name()
        prog.add_inst(AsmModule.Xori(v_name, name, 1))
        return v_name
