...
Original file is located at
    https://colab.research.google.com/drive/1ncDwy63aMQy6xmyxX8LMsRvNdGDjxB2O
...
def percent(df1, df2, alpha):
  ...
  median1 = np.median(taxa1)
  median2 = np.median(taxa2)
...
def t_obs(data):
    ...
    t_stat, _ = ttest_ind(suns_salaries,hornets_salaries,  equal_var=False)

    return t_stat

result = t_obs(data)
assert_equal(round(result, 2), -1.17)
...
def resposta(data):
    ...
    # Separar os dados em dois grupos com base no time
    salarios_hornets = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
    salarios_suns = data[data['TEAM'] == 'Phoenix Suns']['SALARY']

    # Executar o teste t para amostras independentes
    t_stat, p_value = ttest_ind(salarios_hornets, salarios_suns, equal_var=False)

    # Retornar True se a diferença puder ser explicada pelo acaso (p_value >= alpha)
    # Retornar False se houver diferença significativa entre os salários (p_value < alpha)
...
def t_obs2(data):
    alturas_cle = data[data['team_abbreviation'] == 'CLE']['player_height']
    alturas_gsw = data[data['team_abbreviation'] == 'GSW']['player_height']

    dif_medias = alturas_cle.mean() - alturas_gsw.mean()
...
def critical_values(diffs):
  ...
  critico_superior = np.percentile(diffs, (1 - alpha / 2) * 100)
...
def p_value(t_obs, diffs):
    extreme_count = np.sum(np.abs(diffs) >= np.abs(t_obs))
    p_val = extreme_count / len(diffs)
    return p_val

result = t_obs2(df)
foo = p_value(result, diffs)
assert_equal(foo, 0.5392)
...
def resposta2(diffs, t_obs):
  ...
  if np.abs(t_obs) >= np.abs(np.percentile(diffs, (1 - alpha / 2) * 100)):
