...
Original file is located at
    https://colab.research.google.com/drive/1xamSLN-5Yw7fz0Q9HuqjPZQeb3bYq0tf
...
def mean_error(alpha, beta, x, y):
  erro_medio = error(alpha, beta, x, y).mean()
  return erro_medio
...
y_n = predict(alpha, beta, x)

plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x, y_n, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
def sum_of_squared_errors(alpha, beta, x, y):
  # deve retornar a soma dos erros quadrados da saída prevista pelo modelo
  erro_quadrado = (error(alpha, beta, x, y) ** 2).sum()
...
def least_squares_fit(x, y):
  ...
  correlationxy = ss.pearsonr(x, y)

  beta = (correlationxy[0] * np.std(y, ddof=1)) / np.std(x, ddof=1)
...
def r_squared(alpha, beta, x, y):
  # deve retornar o valor do R-quadrado
  R = ((y-y.mean())**2).sum()
  return 1 - sum_of_squared_errors(alpha, beta, x, y)/R
