...
Original file is located at
    https://colab.research.google.com/drive/1pbrl3pq8ltam-i_aPkggYaT_JDMJLhEi
...
def north_or_south():
    verao_sul = ['Dez', 'Jan', 'Fev']
    inverno_sul = ['Jun', 'Jul', 'Ago']

    produtos_verao = ['icecream', 'sunglasses']
    produtos_inverno = ['coats']

    media_verao_pv = df.loc[verao_sul, produtos_verao].mean().mean()
    media_verao_pi = df.loc[verao_sul, produtos_inverno].mean().mean()

    media_inverno_pv = df.loc[inverno_sul, produtos_verao].mean().mean()
    media_inverno_pi = df.loc[inverno_sul, produtos_inverno].mean().mean()

    if media_verao_pv > media_inverno_pv and media_verao_pi < media_inverno_pi:
        return 'Sul'
    else:
        return 'Norte'
...
def mortes_por_pump(df):
    return df.groupby('NearestPumpID').agg({'Count': 'sum'})

print(mortes_por_pump(df))
